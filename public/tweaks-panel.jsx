...
function TweakNumber({ label, value, min, max, step = 1, unit = '', onChange }) {
  ...
  return (
    <div className="twk-num" style={{ display:'flex', alignItems:'center', height:26, padding:'0 0 0 8px', border:'.5px solid rgba(0,0,0,.1)', borderRadius:7, background:'rgba(255,255,255,.6)' }}>
      <span style={{ fontWeight:500, color:'rgba(41,38,27,.6)', paddingRight:8 }}>{label}</span>
      <input type="number" value={value} min={min} max={max} step={step} onChange={(e) => onChange(clamp(Number(e.target.value)))}
        style={{ flex:1, minWidth:0, height:'100%', border:0, background:'transparent', font:'inherit', textAlign:'right', padding:'0 8px 0 0', outline:'none' }} />
...
function TweakColor({ label, value, options, onChange }) {
  if (!options || !options.length) {
    return (
      <div className="twk-row twk-row-h">
        <div className="twk-lbl"><span>{label}</span></div>
        <input type="color" style={{ width:56, height:22, border:'.5px solid rgba(0,0,0,.1)', borderRadius:6, padding:0 }} value={value} onChange={(e) => onChange(e.target.value)} />
      </div>
    );
  }
